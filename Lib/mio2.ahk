...
ReadIni()                                         ; Read .ini file to load port settings - also set up the tray Menu
  {
    ...
    IfExist, AHKTEST.ini
      {
        IniRead, MidiInDevice, AHKTEST.ini, Settings, MidiInDevice , %MidiInDevice%            ; read the midi In port from ini file
        IniRead, MidiOutDevice, AHKTEST.ini, Settings, MidiOutDevice , %MidiOutDevice%   ; read the midi out port from ini file
      }
...
WriteIni()                                                                  ; Write selections to .ini file 
  {
    global MidiInDevice, MidiOutDevice, version
      IfNotExist, AHKTEST.ini                                   ; if no .ini 
        FileAppend,, AHKTEST.ini                              ; make  .ini with the following entries.
    IniWrite, %MidiInDevice%, AHKTEST.ini, Settings, MidiInDevice
    IniWrite, %MidiOutDevice%, AHKTEST.ini, Settings, MidiOutDevice
  }
...
MidiOut:
  global h_midiout                                                                 ; label to load new settings from midi out menu item
  OpenCloseMidiAPI()
...
*/
    ...
    If (result or ErrorLevel)  {
        ;OutputDebug, There was an Error Sending the midi event: (%result%`, %ErrorLevel%)
        Return -1
      }
...
clostmidi(){
  global Mididevz
	midiOutClose(h_midiout)
  result := DllCall("winmm.dll\midiInStop", UInt,Mididevz)
  result := DllCall("winmm.dll\midiInClose", UInt,Mididevz)
  OpenCloseMidiAPI()
}
